...
- **Lazy reconcile**: Cached folders (DB has entries) defer the reconcile walk 3s at `BACKGROUND_SCAN` (10). The heatmap fast path uses trust-cache DB lookups (`get_cached_thumbnail_paths`) — no `os.stat()` on source files. New folders keep immediate walk at Priority(80).
