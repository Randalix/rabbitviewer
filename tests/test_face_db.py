...
class TestFaceDetectionCRUD:
    def test_insert_and_get(self, tmp_env):
        ...
        faces = db.faces.get_faces_for_file("/test/img.jpg")
        assert len(faces) == 1
    ...
    def test_multiple_faces_per_file(self, tmp_env):
        db: MetadataDatabase = tmp_env["db"]
        db.faces.insert_face_detection("f1", "/img.jpg", _fake_embedding(val=0.1),
                                 (0.1, 0.1, 0.2, 0.2), 0.9, "buffalo_l")
        db.faces.insert_face_detection("f2", "/img.jpg", _fake_embedding(val=0.2),
                                 (0.5, 0.5, 0.2, 0.2), 0.85, "buffalo_l")
        faces = db.faces.get_faces_for_file("/img.jpg")
        assert len(faces) == 2

    def test_get_faces_for_empty_file(self, tmp_env):
        db: MetadataDatabase = tmp_env["db"]
        assert db.faces.get_faces_for_file("/no/such/file.jpg") == []


class TestPersonCRUD:
    def test_create_person(self, tmp_env):
        db: MetadataDatabase = tmp_env["db"]
        assert db.faces.create_person("p1", "Alice", "face-1")
        persons = db.faces.get_all_persons()
    ...
    def test_rename_person(self, tmp_env):
        db: MetadataDatabase = tmp_env["db"]
        db.faces.create_person("p1", "Alice")
        db.faces.rename_person("p1", "Bob")
        persons = db.faces.get_all_persons()
    ...
    def test_hide_person(self, tmp_env):
        db: MetadataDatabase = tmp_env["db"]
        db.faces.create_person("p1", "Alice")
        db.faces.hide_person("p1", True)
        assert db.faces.get_all_persons() == []
        assert len(db.faces.get_all_persons(include_hidden=True)) == 1
...
class TestFaceAssignment:
    def test_assign_face_to_person(self, tmp_env):
        db: MetadataDatabase = tmp_env["db"]
        db.faces.create_person("p1", "Alice")
        db.faces.insert_face_detection("f1", "/img.jpg", _fake_embedding(),
                                 (0.1, 0.1, 0.2, 0.2), 0.9, "buffalo_l")
        db.faces.assign_face_to_person("f1", "p1")
    ...
    def test_get_face_paths_for_person(self, tmp_env):
        db: MetadataDatabase = tmp_env["db"]
        db.faces.create_person("p1", "Alice")
        db.faces.insert_face_detection("f1", "/a.jpg", _fake_embedding(val=0.1),
                                 (0.1, 0.1, 0.2, 0.2), 0.9, "buffalo_l")
        db.faces.insert_face_detection("f2", "/b.jpg", _fake_embedding(val=0.2),
                                 (0.1, 0.1, 0.2, 0.2), 0.9, "buffalo_l")
        db.faces.assign_face_to_person("f1", "p1")
        db.faces.assign_face_to_person("f2", "p1")
        paths = db.faces.get_face_paths_for_person("p1")
...
class TestMergePersons:
    def test_merge_reassigns_faces(self, tmp_env):
        db: MetadataDatabase = tmp_env["db"]
        db.faces.create_person("p1", "Alice")
        db.faces.create_person("p2", "Bob")
        db.faces.insert_face_detection("f1", "/a.jpg", _fake_embedding(val=0.1),
                                 (0.1, 0.1, 0.2, 0.2), 0.9, "buffalo_l")
        db.faces.insert_face_detection("f2", "/b.jpg", _fake_embedding(val=0.2),
                                 (0.1, 0.1, 0.2, 0.2), 0.9, "buffalo_l")
        db.faces.assign_face_to_person("f1", "p1")
        db.faces.assign_face_to_person("f2", "p2")

        db.faces.merge_persons("p1", ["p2"])
        ...
        # p2 should be gone
        faces_p2 = db.faces.get_faces_for_person("p2")
...
class TestCascadeDelete:
    def test_remove_records_deletes_faces(self, tmp_env):
        ...
        db.faces.insert_face_detection("f1", fp, _fake_embedding(),
                                 (0.1, 0.1, 0.2, 0.2), 0.9, "buffalo_l")
        assert len(db.faces.get_faces_for_file(fp)) == 1

        db.remove_records([fp])
        assert db.faces.get_faces_for_file(fp) == []


class TestGetFilesMissingFaces:
    def test_returns_missing(self, tmp_env):
        db: MetadataDatabase = tmp_env["db"]
        db.faces.insert_face_detection("f1", "/a.jpg", _fake_embedding(),
                                 (0.1, 0.1, 0.2, 0.2), 0.9, "buffalo_l")
        missing = db.faces.get_files_missing_faces(["/a.jpg", "/b.jpg", "/c.jpg"])
        assert set(missing) == {"/b.jpg", "/c.jpg"}

    def test_empty_input(self, tmp_env):
        db: MetadataDatabase = tmp_env["db"]
        assert db.faces.get_files_missing_faces([]) == []
...
class TestGetFacePathsForPersons:
    def test_union_paths(self, tmp_env):
        ...
        db.faces.insert_face_detection("f2", "/b.jpg", _fake_embedding(val=0.2),
                                 (0.1, 0.1, 0.2, 0.2), 0.9, "buffalo_l")
        db.faces.insert_face_detection("f3", "/c.jpg", _fake_embedding(val=0.3),
                                 (0.1, 0.1, 0.2, 0.2), 0.9, "buffalo_l")
        db.faces.assign_face_to_person("f1", "p1")
        db.faces.assign_face_to_person("f2", "p2")
        db.faces.assign_face_to_person("f3", "p1")
        paths = db.faces.get_face_paths_for_persons(["p1", "p2"])
        assert set(paths) == {"/a.jpg", "/b.jpg", "/c.jpg"}

    def test_empty_ids(self, tmp_env):
        db: MetadataDatabase = tmp_env["db"]
        assert db.faces.get_face_paths_for_persons([]) == []
    ...
    def test_single_person(self, tmp_env):
        ...
        db.faces.insert_face_detection("f1", "/a.jpg", _fake_embedding(),
                                 (0.1, 0.1, 0.2, 0.2), 0.9, "buffalo_l")
        db.faces.assign_face_to_person("f1", "p1")
        paths = db.faces.get_face_paths_for_persons(["p1"])
        assert paths == ["/a.jpg"]
...
class TestPersonNames:
    def test_get_person_names(self, tmp_env):
        ...
        db.faces.create_person("p1", "Alice")
        db.faces.create_person("p2", "Bob")
        db.faces.create_person("p3", "")  # unnamed
        names = db.faces.get_person_names()
        assert names == ["Alice", "Bob"]

    def test_get_person_by_name(self, tmp_env):
        db: MetadataDatabase = tmp_env["db"]
        db.faces.create_person("p1", "Alice")
        person = db.faces.get_person_by_name("Alice")
        assert person is not None
        assert person['person_id'] == "p1"
        assert db.faces.get_person_by_name("NoSuchPerson") is None
