...
class TestTagCRUD:
    ...
    def test_get_or_create_tag_creates(self, tmp_env):
        db = tmp_env["db"]
        tag_id = db.tags.get_or_create_tag("landscape")
        assert tag_id > 0
    ...
    def test_get_or_create_tag_idempotent(self, tmp_env):
        db = tmp_env["db"]
        id1 = db.tags.get_or_create_tag("portrait")
        id2 = db.tags.get_or_create_tag("portrait")
        assert id1 == id2
    ...
    def test_add_image_tags_deduplicates(self, tmp_env, sample_images):
        ...
        path = sample_images[0]
        db.images.batch_ensure_records_exist([path])
        db.tags.add_image_tags(path, ["sunset"])
        db.tags.add_image_tags(path, ["sunset", "beach"])
        tags = db.tags.get_image_tags(path)
        assert sorted(tags) == ["beach", "sunset"]
    ...
    def test_remove_image_tags(self, tmp_env, sample_images):
        ...
        path = sample_images[0]
        db.images.batch_ensure_records_exist([path])
        db.tags.add_image_tags(path, ["a", "b", "c"])
        db.tags.remove_image_tags(path, ["b"])
        assert sorted(db.tags.get_image_tags(path)) == ["a", "c"]

    def test_set_image_tags_replaces(self, tmp_env, sample_images):
        db = tmp_env["db"]
        path = sample_images[0]
        db.images.batch_ensure_records_exist([path])
        db.tags.add_image_tags(path, ["old_tag"])
        db.tags.set_image_tags(path, ["new_tag"])
        assert db.tags.get_image_tags(path) == ["new_tag"]

    def test_get_image_tags_empty(self, tmp_env, sample_images):
        db = tmp_env["db"]
        path = sample_images[0]
        db.images.batch_ensure_records_exist([path])
        assert db.tags.get_image_tags(path) == []
...
class TestTagBatchOps:
    ...
    def test_batch_set_tags(self, tmp_env, sample_images):
        ...
        ok = db.tags.batch_set_tags(paths, ["hero", "select"])
        assert ok is True
        for p in paths:
            assert sorted(db.tags.get_image_tags(p)) == ["hero", "select"]

    def test_batch_remove_tags(self, tmp_env, sample_images):
        db = tmp_env["db"]
        paths = sample_images[:3]
        db.images.batch_ensure_records_exist(paths)
        db.tags.batch_set_tags(paths, ["keep", "remove_me"])
        ok = db.tags.batch_remove_tags(paths, ["remove_me"])
        assert ok is True
        for p in paths:
            assert db.tags.get_image_tags(p) == ["keep"]

    def test_batch_set_empty_returns_false(self, tmp_env):
        db = tmp_env["db"]
        assert db.tags.batch_set_tags([], ["tag"]) is False
        assert db.tags.batch_set_tags(["path"], []) is False
...
class TestTagListing:
    ...
    def test_get_all_tags(self, tmp_env, sample_images):
        db = tmp_env["db"]
        db.images.batch_ensure_records_exist(sample_images[:2])
        db.tags.add_image_tags(sample_images[0], ["alpha"])
        db.tags.add_image_tags(sample_images[1], ["beta"])
        names = [t["name"] for t in db.tags.get_all_tags()]
        assert "alpha" in names
        assert "beta" in names

    def test_get_all_tags_by_kind(self, tmp_env, sample_images):
        db = tmp_env["db"]
        db.images.batch_ensure_records_exist([sample_images[0]])
        # Create tags with different kinds
        db.tags.get_or_create_tag("select", kind="workflow")
        db.tags.get_or_create_tag("landscape", kind="keyword")
        workflow = db.tags.get_all_tags(kind="workflow")
        keyword = db.tags.get_all_tags(kind="keyword")
    ...
    def test_get_directory_tags(self, tmp_env, sample_images):
        db = tmp_env["db"]
        db.images.batch_ensure_records_exist(sample_images[:3])
        db.tags.add_image_tags(sample_images[0], ["dir_tag"])
        # Get tags for the parent directory
        parent = os.path.dirname(sample_images[0])
        dir_tags = db.tags.get_directory_tags(parent)
...
class TestFilteredFilePathsWithTags:
    ...
    def test_filter_by_tags(self, tmp_env, sample_images):
        ...
        paths = sample_images[:5]
        db.images.batch_ensure_records_exist(paths)
        db.tags.add_image_tags(paths[0], ["hero"])
        db.tags.add_image_tags(paths[1], ["hero", "select"])
        db.tags.add_image_tags(paths[2], ["reject"])

        all_stars = [True, True, True, True, True, True]
        result = db.images.get_filtered_file_paths("", all_stars, tag_names=["hero"])
    ...
    def test_filter_by_tags_and_rating(self, tmp_env, sample_images):
        ...
        paths = sample_images[:3]
        db.images.batch_ensure_records_exist(paths)
        db.images.batch_set_ratings(paths[:2], 3)
        db.tags.add_image_tags(paths[0], ["hero"])
        db.tags.add_image_tags(paths[1], ["hero"])
        db.tags.add_image_tags(paths[2], ["hero"])

        # Only rating 3 + tag "hero"
        star_states = [False, False, False, True, False, False]  # only 3-star
        result = db.images.get_filtered_file_paths("", star_states, tag_names=["hero"])
        assert sorted(result) == sorted([paths[0], paths[1]])
    ...
    def test_filter_no_tags_returns_all(self, tmp_env, sample_images):
        ...
        db.images.batch_ensure_records_exist(paths)
        all_stars = [True, True, True, True, True, True]
        result = db.images.get_filtered_file_paths("", all_stars, tag_names=None)
        assert len(result) == 3
    ...
    def test_filter_nonexistent_tag_returns_empty(self, tmp_env, sample_images):
        ...
        db.images.batch_ensure_records_exist(paths)
        all_stars = [True, True, True, True, True, True]
        result = db.images.get_filtered_file_paths("", all_stars, tag_names=["nonexistent"])
        assert result == []
