...
class TestFileWorkBatchInsert:
    ...
    def test_inserts_rows(self, db):
        ...
        result = db.ledgers.file_work_get_pending("/photos", "thumbnail")
        assert sorted(result) == ["/a.jpg", "/b.jpg"]

    def test_idempotent(self, db):
        db.ledgers.file_work_batch_insert(["/a.jpg"], "thumbnail", "/photos")
        db.ledgers.file_work_batch_insert(["/a.jpg"], "thumbnail", "/photos")
        result = db.ledgers.file_work_get_pending("/photos", "thumbnail")
        assert result == ["/a.jpg"]

    def test_empty_list_is_noop(self, db):
        db.ledgers.file_work_batch_insert([], "thumbnail", "/photos")
        assert db.ledgers.file_work_get_all_roots() == []

    def test_multiple_work_types(self, db):
        db.ledgers.file_work_batch_insert(["/a.jpg"], "thumbnail", "/photos")
        db.ledgers.file_work_batch_insert(["/a.jpg"], "clip", "/photos")
        types = sorted(db.ledgers.file_work_get_pending_types("/photos"))
...
class TestFileWorkRemove:
    ...
    def test_remove_nonexistent_is_noop(self, db):
        db.ledgers.file_work_remove("/nonexistent.jpg", "thumbnail")

    def test_remove_wrong_type_leaves_other(self, db):
        db.ledgers.file_work_batch_insert(["/a.jpg"], "thumbnail", "/photos")
        db.ledgers.file_work_remove("/a.jpg", "clip")
        assert db.ledgers.file_work_get_pending("/photos", "thumbnail") == ["/a.jpg"]


class TestFileWorkBatchRemove:

    def test_removes_multiple(self, db):
        db.ledgers.file_work_batch_insert(["/a.jpg", "/b.jpg", "/c.jpg"], "clip", "/photos")
        db.ledgers.file_work_batch_remove(["/a.jpg", "/c.jpg"], "clip")
        assert db.ledgers.file_work_get_pending("/photos", "clip") == ["/b.jpg"]

    def test_empty_list_is_noop(self, db):
        db.ledgers.file_work_batch_remove([], "clip")
...
class TestFileWorkQueries:
    ...
    def test_get_all_roots(self, db):
        db.ledgers.file_work_batch_insert(["/a.jpg"], "thumbnail", "/photos")
        db.ledgers.file_work_batch_insert(["/b.jpg"], "clip", "/videos")
        roots = sorted(db.ledgers.file_work_get_all_roots())
    ...
    def test_get_pending_types(self, db):
        db.ledgers.file_work_batch_insert(["/a.jpg"], "thumbnail", "/photos")
        db.ledgers.file_work_batch_insert(["/a.jpg"], "view_image", "/photos")
        db.ledgers.file_work_batch_insert(["/a.jpg"], "clip", "/photos")
        types = sorted(db.ledgers.file_work_get_pending_types("/photos"))
        assert types == ["clip", "thumbnail", "view_image"]

    def test_get_pending_types_empty(self, db):
        assert db.ledgers.file_work_get_pending_types("/nonexistent") == []

    def test_get_pending_filters_by_root_and_type(self, db):
        db.ledgers.file_work_batch_insert(["/a.jpg"], "thumbnail", "/photos")
        db.ledgers.file_work_batch_insert(["/b.jpg"], "thumbnail", "/videos")
        assert db.ledgers.file_work_get_pending("/photos", "thumbnail") == ["/a.jpg"]
        assert db.ledgers.file_work_get_pending("/videos", "thumbnail") == ["/b.jpg"]

    def test_same_file_two_roots(self, db):
        """A file discovered under two scan roots keeps both entries."""
        db.ledgers.file_work_batch_insert(["/a.jpg"], "thumbnail", "/photos")
        db.ledgers.file_work_batch_insert(["/a.jpg"], "thumbnail", "/backup")
        assert db.ledgers.file_work_get_pending("/photos", "thumbnail") == ["/a.jpg"]
        assert db.ledgers.file_work_get_pending("/backup", "thumbnail") == ["/a.jpg"]
        assert sorted(db.ledgers.file_work_get_all_roots()) == ["/backup", "/photos"]
...
class TestFileWorkClearAll:
    ...
    def test_clears_everything(self, db):
        db.ledgers.file_work_batch_insert(["/a.jpg"], "thumbnail", "/photos")
        db.ledgers.file_work_batch_insert(["/b.jpg"], "clip", "/videos")
        cleared = db.ledgers.file_work_clear_all()
        assert cleared == 2
        assert db.ledgers.file_work_get_all_roots() == []

    def test_clear_empty_returns_zero(self, db):
        assert db.ledgers.file_work_clear_all() == 0
