...
def run_script(api: ScriptAPI, selected_images: list[str] = None):
    """
    Sets the rating of the selected images/folders to 3 stars.
    ...
    if not selected_images and not folders:
        logger.info("No images or folders selected to set rating to 3 stars.")
        return

    if selected_images:
        logger.info(f"Setting rating to 3 stars for {len(selected_images)} images.")
        api.set_rating_for_images(selected_images, 3)
        api.show_overlay(selected_images, "stars", {"count": 3}, duration=2000, overlay_id="rating")

    if folders:
        logger.info(f"Setting rating to 3 stars for {len(folders)} folders.")
        api.set_rating_for_folders(folders, 3)

    logger.info("Rating set to 3 stars.")
