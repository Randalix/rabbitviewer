...
def run_script(api: ScriptAPI, selected_images: list[str] = None):
    ...
    if not selected_images:
        logging.info("No images selected to set rating to 2 stars.")
        return

    logging.info(f"Setting rating to 2 stars for {len(selected_images)} images.")
    api.set_rating_for_images(selected_images, 2)
    api.show_overlay(selected_images, "stars", {"count": 2}, duration=1200)
    logging.info("Rating set to 2 stars.")
