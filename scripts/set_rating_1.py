...
def run_script(api: ScriptAPI, selected_images: list[str] = None):
    """
    Sets the rating of the selected images/folders to 1 star.
    ...
    if not selected_images and not folders:
        logger.info("No images or folders selected to set rating to 1 star.")
        return

    if selected_images:
        logger.info(f"Setting rating to 1 star for {len(selected_images)} images.")
        api.set_rating_for_images(selected_images, 1)
        api.show_overlay(selected_images, "stars", {"count": 1}, duration=2000, overlay_id="rating")

    if folders:
        logger.info(f"Setting rating to 1 star for {len(folders)} folders.")
        api.set_rating_for_folders(folders, 1)

    logger.info("Rating set to 1 star.")
